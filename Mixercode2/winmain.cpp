...
// Track current pan/vol for display (0..255); seeded after mixer_init.
static int  g_curPan[8] = {};
static int  g_curVol[8] = {};
...
// Levels are pulled from mixer_get_vu(). Coordinates in current ortho (pixel) space.
// -----------------------------------------------------------------------------
static void DrawVUMeters(int x, int y, int w, int h)
{
	glDisable(GL_TEXTURE_2D);
	...
	// Labels (caller wraps the HUD in StartTextMode/EndTextMode).
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int)
{
	...
	g_chanSampleId[0] = loadSampleFromFileOrZip(0, "data\\musicstereo.wav");
	g_chanSampleId[1] = loadSampleFromFileOrZip(0, "data\\InGame1Loop.wav");
	g_chanSampleId[2] = loadSampleFromFileOrZip(0, "data\\sfx_zap.wav");
	g_chanSampleId[3] = loadSampleFromFileOrZip(0, "data\\fire.wav");
	g_chanSampleId[4] = loadSampleFromFileOrZip(0, "data\\Natural Vibes.mp3");
	g_chanSampleId[5] = loadSampleFromFileOrZip(0, "data\\sfx_lose.ogg");
	g_chanSampleId[6] = loadSampleFromFileOrZip(0, "data\\drone_22050.wav");
	g_chanSampleId[7] = loadSampleFromFileOrZip(0, "data\\strings2mono.wav");
	...
	while (!quit)
	{
		...
		else
		{
			...
			// Per-channel rows
			for (int ch = 0; ch < 8; ++ch) {
				const int playing = sample_playing(ch);
	...
	// ---- Shutdown ----
	// Free GL resources while the context is still current.
	if (g_bgTex) {
