...
int WINAPI WinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_  LPSTR lpCmdLine, _In_ int iCmdShow)
{
	...
	// create main window
	hWnd = CreateWindow(L"GLSample", L"OpenGL Sample", WS_CAPTION | WS_POPUPWINDOW | WS_VISIBLE, 0, 0, SCREEN_W, SCREEN_H, NULL, NULL, hInstance, NULL);
	...
	load_sample(0, "data\\musicstereo.wav");
	load_sample(0, "data\\InGame1Loop.wav");
	load_sample(0, "data\\sfx_zap.wav");
	load_sample(0, "data\\fire.wav");
	load_sample(0, "data\\whoosh.mp3");
	load_sample(0, "data\\sfx_lose.ogg");
	load_sample(0, "data\\drone_22050.wav");
	...
	while (!quit)
	{
		...
		else
		{
			...
			if (KeyCheck(KEY_4)) {
				                  //sample_start(4, 4, 0); 
				                  sample_start(4, nameToNum("whoosh"), 0);
			                     }
